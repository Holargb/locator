...
import scipy.stats
from . import losses
from .comparison import plot_bmm_otsu_comparison
...
def make_metric_plots(csv_path, taus, radii, title=''):
    ...
    # 添加BMM和Otsu对比图
    if -2 in taus and -1 in taus:  # 确保两种方法数据都存在
        fig = plot_bmm_otsu_comparison(csv_path, radii, title)
        dic['bmm_vs_otsu_comparison'] = fig
        plt.close(fig)
