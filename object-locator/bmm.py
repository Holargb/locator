...
def _get_values_test(x, left, right):
    # Convert x to tensor
...
def get_initialization_test(x, ncomponents, limit=0.8):
    # 使用 GMM 对数据进行初始化
    gmm = GaussianMixture(n_components=ncomponents)
    gmm.fit(x.reshape(-1, 1))  # GMM 训练
...
def get_weights_test(x, ab, pi):
    """return nsamples X ncomponents matrix with association weights"""
    bpdf = beta.pdf
    n, c = len(x), len(ab)
    y = torch.zeros((n, c), dtype=torch.float32)  # Use torch instead of numpy
    s = torch.zeros((n, 1), dtype=torch.float32)  # Use torch instead of numpy
    for (j, p, (a, b)) in zip(count(), pi, ab):
        y[:, j] = p * torch.tensor(beta.pdf(x, a, b), dtype=torch.float32)  # Convert to tensor
    s = torch.sum(y, 1).reshape((n, 1))

    w = y / s  # PyTorch element-wise division
    w = torch.nan_to_num(w, nan=0.0, posinf=0.0, neginf=0.0)  # PyTorch way to handle NaN and Inf

    # Clean up weights w, remove infs, nans, etc.
    wfirst = torch.tensor([1] + [0] * (c - 1), dtype=torch.float32)
    wlast = torch.tensor([0] * (c - 1) + [1], dtype=torch.float32)
    bad = (~torch.isfinite(w)).any(dim=1)
    badfirst = torch.logical_and(bad, x < 0.5)
    badlast = torch.logical_and(bad, x >= 0.5)
    w[badfirst, :] = wfirst
    w[badlast, :] = wlast
    return w
