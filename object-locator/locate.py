    ...
    for i in range(mask.shape[0]):
        for j in range(mask.shape[1]):
            if mask[i, j] == 1 and labeled[i, j] == 0:
                labeled[mask == 1] = current_label
                current_label += 1
    
    # 修改为两步操作
    y = torch.arange(mask.shape[0], device=mask.device)
    x = torch.arange(mask.shape[1], device=mask.device)
...
def gpu_paint_circles(img_tensor, points, radius=3, color='red'):
    """
    GPU绘制圆点
    输入: 
        img_tensor [C, H, W] (GPU Tensor)
        points [N, 2] (y, x坐标)
    输出: 带标记的图像 [C, H, W]
    """
    # 创建圆形模板
    y, x = torch.meshgrid(torch.arange(img_tensor.shape[1], device=img_tensor.device),
                          torch.arange(img_tensor.shape[2], device=img_tensor.device))
    circle_mask = torch.zeros_like(img_tensor[0], dtype=torch.uint8)  # 使用uint8替代
    circle_mask = circle_mask.bool() if hasattr(torch, 'bool') else circle_mask.byte()
    
    for (y_center, x_center) in points:
        dist = ((y - y_center)**2 + (x - x_center)**2).sqrt()
        circle_mask |= (dist <= radius)
    
    # 根据颜色填充
    if color == 'red':
        img_tensor[0][circle_mask] = 1.0  # R通道
        img_tensor[1][circle_mask] = 0.0  # G通道
        img_tensor[2][circle_mask] = 0.0  # B通道
    return img_tensor
...
testset_loader = data.DataLoader(testset,
                                 batch_size=args.batch_size,  # 增大batch_size
...
for batch_idx, (imgs, dictionaries) in tqdm(enumerate(testset_loader),
    ...
    '''est_map_np_origsize = skimage.transform.resize(est_map_np,
                                 output_shape=origsize,
                                 mode='constant')'''
    
    # GPU上双线性插值缩放
    # 明确指定align_corners=True
    est_map_np_origsize = F.interpolate(
    ...
    for t, tau in enumerate(args.taus):
        if tau != -2:
            ...
            mask = (est_map_np_origsize > tau).float()  # 输出二值mask [B, H, W]
        else:
            mask, _, mix = utils.threshold(est_map_np_origsize, tau)
            bmm_tracker.feed(mix)
        centroids_wrt_orig = find_blob_centroids_gpu(mask[0])  # 处理batch中的第一个样本
        ...
        if args.paint:
            ...
            img_with_x = gpu_paint_circles(orig_img_np_origsize, centroids_wrt_orig)
